...
bool WhiskerTree::combine_other(const WhiskerTree& other, bool trace){
    if( !(_domain == other._domain)){
        fprintf(stderr,"whiskers domain error:\n%s\n%s\n",_domain.str().c_str(), other._domain.str().c_str());
        return false;
    }
    if(is_leaf()){
        if(!other.is_leaf()){
            fprintf(stderr,"other is not leaf\n");
            return false;

        }
        //_leaf.front()._domain.combine_other(other._leaf.front().domain(), trace);
        return _leaf.front().combine_other(other._leaf.front() , trace);
    }
    else{
        if(other._children.size()!=_children.size()){
            fprintf(stderr,"children_size: %lu != %lu",_children.size(),other._children.size());
            return false;
        }
        for(size_t i = 0; i<_children.size();++i){
            if(! _children[i].combine_other(other._children[i], trace) )return false;
        }
    }
    return true;
}
