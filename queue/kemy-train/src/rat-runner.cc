...
int main( int argc, char *argv[] )
{
  ...
  configuration_range.bottle_bw = make_pair( link_ppt, 0 ); /* 1 Mbps to 10 Mbps */
  configuration_range.bottle_single_delay = make_pair( delay, 0 ); /* ms */
...
/*  configuration_range.mean_on_duration = mean_on_duration;*/
  ...
  auto outcome = eval.score( whiskers, false );
  printf( "score = %f\n", outcome.score );
  double norm_score = 0;
