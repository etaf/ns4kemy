...
/*  for ( unsigned int i = (range.max_senders+1)/3*2; i <= range.max_senders; ++i)*/
  ...
  for ( int i = 0; i < 15; i++ ) {
    ...
    _configs.push_back( NetConfig().set_bottle_bw( link_speed(global_PRNG()) ).set_bottle_single_delay( range.bottle_single_delay.first ).set_num_senders( range.max_senders ) );
  }
}
