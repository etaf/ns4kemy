...
{
  return std::accumulate( _gang.begin(), _gang.end(),
							   ...
							   const unsigned int num_sending __attribute((unused)) )
{
  ...
  while ( SwitchedSender::next_switch_tick <= tickno ) {
    ...
    SwitchedSender::next_switch_tick = std::numeric_limits<double>::max();
...
template <class SenderType>
std::vector< std::pair< double, double > > SenderGang<SenderType>::throughputs_delays( void ) const
{
...
double SenderGang<SenderType>::SwitchedSender::next_event_time( const double & tickno ) const
{
  ...
  return std::min( next_switch_tick, sending ? sender.next_event_time( tickno ) : std::numeric_limits<double>::max() );
}
